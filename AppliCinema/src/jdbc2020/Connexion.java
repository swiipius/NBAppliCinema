...
public class Connexion {
    ...
    //Requete sql dont le resutltat est plusieurs String[] contenue dans une arraylist pour faciliter l'extraction de donnée
    public DefaultListModel requestDemande(String requete) throws SQLException {
        // récupération de l'ordre de la requete
        rset = stmt.executeQuery(requete);
        ...
        while (rset.next()) {
            ...
            // Concatener les champs de la ligne separes par ,
            for (int i = 1; i < nbColonne; i++) {
                Resultat.addElement(rset.getString(i + 1));
            }

            // ajouter les champs de la ligne dans l'ArrayList
            Resultat.addElement(champs);
        }
